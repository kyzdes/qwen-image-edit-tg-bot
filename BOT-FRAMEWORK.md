...
| **+/- & toggle settings** | Numeric or boolean params the user may tune | Row of `[ − ] [ current value ] [ + ]` per number; a toggle button per boolean | `steps` and `cfg` stepper rows, a randomize-seed toggle, a "set seed" button |
...
- **The current value lives *in* the keyboard**, between the − and + buttons (`[−] [4] [+]`). The user reads state off the keyboard itself; you don't need a separate "current settings" message.
...
Below is a summary of the substantive fixes I made (the document above is the complete, ready-to-save version).
...
- Added the `seed 0..2147483647` clamp to the Settings code, the Phase 4 done-check, and the Phase 6.4 verification gate (it was in the contract table but missing from the actual clamp code/checks).
- Corrected the hardcoded `image/jpeg` MIME claim: it's safe for Telegram *photos* (always JPEG) but image *documents* can be PNG — added the caveat to derive MIME from `mime_type` for the document path.
- Made the 64-byte assertion explicit about UTF-8 byte length (`callback_data.encode("utf-8")`), since the doc itself stresses bytes not chars.
- Tightened "GPU-seconds budget" / "GPU-minutes" wording to "GPU budget" + an explicit note that xlarge ×2 doubles consumption; clarified large/xlarge are quota *multipliers*. Pinned "current versions" → "2.5.0" in the Phase 6.3 review-lens table to match Phase 5.1.

**Gaps made actionable:** Added explicit prerequisites to the New-bot quickstart (secrets-manager access, target id, reachable Dokploy) so step 1 genuinely starts from zero; added `mounts.create type=volume` to Phase 7.3 step 6; folded clamp-assertion into the Phase 6.4 invariants step; filled two empty "Why/evidence" cells in the Gotchas catalog. I verified the H:MM:SS→9543 arithmetic the doc cites is correct.

No factual claim was invented beyond the ground truth; all edits either reconcile internal inconsistencies or add cross-references and the one MIME caveat (which is a correctness guard, not a new feature claim).

...
| **GpuUnavailable** (ZeroGPU slot starvation) | `No GPU was available after 60s` / `GPU task aborted` on a Space that *has* quota and is awake (xlarge needs **2** slots) | **Yes — wait + re-queue** (~8 s, ~2×) | "Backend GPU is busy — retrying…"; if still no slot, "GPU overloaded, try again shortly" — **not** a quota message (gotcha #34) |
...
- [ ] Auto-retried classes are **transient connection** *and* **GPU-slot-starvation** (wait + re-queue); quota and generic are not.
- [ ] Classify **GPU-slot-starvation BEFORE quota**, with narrow markers — a broad `"gpu task"` quota-marker mis-tags `GPU task aborted` as "quota exhausted" (gotcha #34).
...
> **Don't downsize to dodge scheduling.** `xlarge` (2 slots) is harder to schedule than `large` (1 slot) on the shared pool — but if the model *needs* `xlarge` you **can't** just set `size="large"` to make slots easier to get: it OOMs (`GPU task aborted`) even when the summed weights look like they "fit" 48 GB (runtime peak ≫ weight sum). Verify any size change with a **real GPU call**, not arithmetic; keep `xlarge` and absorb the scheduling pain with the GpuUnavailable retry (§5.5) instead (gotchas #33, #34).
...
| 33 | ZeroGPU sizing | A model on `xlarge` **can't** be dropped to `large` to make it easier to schedule — `large` builds and the app starts, but the first GPU call dies with `GPU task aborted` | **OOM at runtime.** Summing weight files ("~37 GB < 48 GB → fits large") misses the runtime peak (activations, attention/KV buffers, decode tensors); the fp8 stack genuinely needs ~2 slots / 96 GB | Verify fit with a **real GPU call**, never weight arithmetic. If the upstream author chose `xlarge`, trust it — keep it (size lives in code: `@spaces.GPU(size=...)`). Accept the harder scheduling over a smaller size that OOMs (Phase 1.4 / Phase 5) | Flipped the Qwen-Image-Edit v2 Space `xlarge`→`large`: built + RUNNING, but `/infer` instantly `GPU task aborted`; reverted to xlarge |
| 34 | ZeroGPU slot starvation | `No GPU was available after 60s. Retry later.` (or `GPU task aborted`) on a Space that has quota left and is already awake | **Not quota, not a crash, not the Space being down** — `xlarge` needs **2** GPU slots at once; on the shared *free* pool the scheduler can't allocate both within its 60 s wait under load. Transient and load-dependent | Treat it as a **retryable** class: wait ~8 s and re-queue up to ~2× before surfacing (a freed slot usually appears). Check this class **before** the quota branch and keep its markers narrow — a broad `"gpu task"` quota-marker mis-tagged `GPU task aborted` as "quota exhausted." Sustained pool saturation still fails — an inherent free-tier limit for an xlarge model (Phase 5 §5.5) | The edit-bot returned this on most generations until the bot retried; fixed in `ac376ba` |
